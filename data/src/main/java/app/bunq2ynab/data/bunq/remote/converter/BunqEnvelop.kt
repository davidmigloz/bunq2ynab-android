...
internal class BunqEnvelopFactory @Inject constructor() : JsonAdapter.Factory {
    ...
    override fun create(
        ...
        return if (type !is ParameterizedType || type.rawType != BunqEnvelop::class.java || annotations.isNotEmpty()) {
            ...
            BunqEnvelopAdapter(
                responseAdapter = moshi.adapter<Any>(type.actualTypeArguments[0]),
            )
        }
    }
            ...
            return try {
                ...
                while (reader.hasNext()) {
                    when (reader.nextName()) {
                        "Response" -> {
                            response = responseAdapter.fromJson(reader) ?: error("Null response")
                        }
                        "Error" -> {
                            reader.skipValue()
                        }
...
//            return@Converter when (response.size) { TODO
//                0 -> null             // Empty list -> treat it as null
//                1 -> response.first() // Only one object -> unwrap object
//                else -> response      // More than one object -> propagate list
//            }
        }
    }
}
